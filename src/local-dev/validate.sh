#!/bin/bash
# Validates that all swim-ed254-provider compose services are working correctly.
...
echo ""
echo "=== swim-ed254-provider — compose validation ==="
echo ""

echo "--- Container status ---"
for name in ed254-provider-postgres kafka ed254-provider-akhq keycloak ed254-provider-artemis; do
...
check "provider-postgres: accepting connections" \
    podman exec ed254-provider-postgres pg_isready -U postgres

echo ""
echo "--- Kafka ---"
check "ed254-events-all-topic exists" \
    "podman exec kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --list 2>/dev/null | grep -q ed254-events-all-topic"
check "ed254-outbox topic exists" \
    "podman exec kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --list 2>/dev/null | grep -q ed254-outbox"

echo ""
echo "--- AKHQ ---"
check "AKHQ UI reachable (port 9091)" \
    "curl -sf http://localhost:9091 -o /dev/null"
...
check "ACKMonitorPlugin loaded (no ClassNotFoundException)" \
    "podman logs ed254-provider-artemis 2>&1 | grep -v ClassNotFoundException | grep -qi 'ACKMonitor\|plugin'"
check "AMQP plain (port 5672): send as admin" \
    "podman exec ed254-provider-artemis /home/jboss/amq-broker/bin/artemis producer \
...
echo ""
echo "--- Validator ---"
check "validator UI reachable (port 8086)" \
    "curl -sf http://localhost:8086 -o /dev/null"
check "validator: no datasource error in logs" \
    "! podman logs ed254-provider-validator 2>&1 | grep -qi 'datasource\|connection refused\|unable to acquire'"
